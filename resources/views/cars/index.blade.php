                    ...
                    <form method="GET" action="{{ route('cars.index') }}">
                        <div class="mb-3">
                            ...
                            <input type="text" name="search" class="form-control form-control-sm"
                                value="{{ $filters['search'] ?? '' }}" placeholder="Brand, model...">
                        </div>
                        <div class="mb-3">
                            <label class="form-label"
                                style="font-size:0.78rem;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:#6c757d">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="category" class="form-select form-select-sm">
                                ...
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat }}"
                                        {{ ($filters['category'] ?? '') === $cat ? 'selected' : '' }}>{{ $cat }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="availability" class="form-select form-select-sm">
                                <option value="available"
                                    {{ ($filters['availability'] ?? 'available') === 'available' ? 'selected' : '' }}>
                                    Available</option>
                                <option value="booked"
                                    {{ ($filters['availability'] ?? '') === 'booked' ? 'selected' : '' }}>Booked</option>
                                <option value="" {{ ($filters['availability'] ?? '') === '' ? 'selected' : '' }}>All
                                </option>
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="number" name="min_price" class="form-control form-control-sm"
                                value="{{ $filters['min_price'] ?? '' }}" placeholder="e.g. 1000">
                        </div>
                        <div class="mb-3">
                            <label class="form-label"
                                style="font-size:0.78rem;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:#6c757d">
                            ...
                            <input type="number" name="max_price" class="form-control form-control-sm"
                                value="{{ $filters['max_price'] ?? '' }}" placeholder="e.g. 10000">
                        </div>
                        <div class="mb-4">
                            <label class="form-label"
                                style="font-size:0.78rem;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:#6c757d">
                            ...
                            <select name="rental_days" class="form-select form-select-sm">
                                ...
                                @foreach ([1, 2, 3, 5, 7, 14, 30] as $d)
                                    <option value="{{ $d }}"
                                        {{ ($filters['rental_days'] ?? '') == $d ? 'selected' : '' }}>{{ $d }}
                                        Day{{ $d > 1 ? 's' : '' }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <a href="{{ route('cars.index') }}" class="btn btn-outline-red btn-sm w-100">
            ...
            <div class="col-lg-9">
                <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
                    <div>
                        ...
                        <p style="color:#6c757d;font-size:0.85rem;margin:0">{{ $cars->total() }} cars found</p>
                    </div>
                </div>
                <div class="row">
                    @forelse($cars as $car)
                        @include('partials.car-card', ['car' => $car, 'filters' => $filters])
                    @empty
                        <div class="col-12">
